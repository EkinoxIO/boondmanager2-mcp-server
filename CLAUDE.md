...
**181 tools, 11 prompts, 22 resources** across 38 domains, allowing Claude
...
- **Release** (`.github/workflows/release.yml`): Triggered on `v*` tags. Publishes to:
  ...
  - **GHCR** (`ghcr.io/EkinoxIO/boondmanager2-mcp-server`) — multi-arch (`linux/amd64`+`linux/arm64`) with provenance + SBOM, tags `:X.Y.Z` `:X.Y` `:X` `:latest`
  - **Docker Hub** (`docker.io/{DOCKERHUB_USERNAME}/boondmanager2-mcp-server`) — same multi-arch image, same tags. Gated on the `DOCKERHUB_TOKEN` repo secret being present, so forks without the secret skip Docker Hub without failing the release.
